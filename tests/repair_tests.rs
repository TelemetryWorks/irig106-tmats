...
fn repair_fixes_g_dsi_counter() {
    ...
    assert!(
        report
            .actions_taken
            .iter()
            .any(|a| a.attribute_path == "G\\DSI\\N")
    );
}
...
fn repair_is_idempotent() {
    ...
    assert!(
        report2.actions_taken.is_empty(),
...
fn repair_dry_run_does_not_modify() {
    ...
    assert!(
        ...
        "should detect the counter mismatch"
    );
    assert!(
        report.actions_taken.is_empty(),
        "dry-run should take no actions"
    );
...
fn repair_reduces_validation_errors() {
    ...
    assert!(
        report.post_repair_errors <= report.pre_repair_errors,
        "repair should not increase errors: pre={}, post={}",
        report.pre_repair_errors,
        report.post_repair_errors
    );
}
...
fn repair_normalizes_keyword_case() {
    ...
    assert!(
        ...
        "should have case normalization actions"
    );

    // Verify the actual values were changed
    let packing = doc.recorders[&1].channels[&1]
        .data_packing_option
        .as_deref();
    ...
    assert_eq!(
        encoding,
        Some("NRZ-L"),
        "encoding should be normalized to uppercase"
    );
}
...
fn repair_detects_orphan_channels() {
    ...
    assert!(
        !orphan_findings[0].auto_fixable,
...
fn repair_no_orphans_when_links_resolve() {
    ...
    assert!(
        orphan_findings.is_empty(),
        "should have no orphans when links resolve"
    );
}
