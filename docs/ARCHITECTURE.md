...
## 9. Refinement T6: edition declarations and the validation basis

From the team design review, priority T6 (`docs/ROADMAP.md`). Applied
2026-09-26 with the owner's choice of a labelled fallback; recorded in
ADR-0028, which partly supersedes ADR-0016.

### 9.1 Two declarations, two questions
...
between them is **not a finding** by default — TMATS written to one edition
may describe a recording that complies with another. A user rule may flag
it (ADR-0006).

### 9.2 The validation basis

![Two declarations, and the rules actually applied](diagrams/edition-basis.svg)

*Choosing the rules.* An override wins. Otherwise the edition `G\106` names
is validated if the registry covers it (106-04 to 106-24R1), or given a
compatibility check, named as such, if it does not. Otherwise — `G\106`
missing or unrecognised — a labelled fallback applies: the edition RCCVER
reads as (for `0x0E`, the newest it covers, INT-017), else the baseline
106-24R1. The report always states the rules applied and their basis:
**override**, **declared**, or **fallback** with its reason.

### 9.3 Compatibility checks

A file declaring an edition the registry does not cover — `G\106` = `96`, or
an edition after the baseline — is never reported as "validated against"
that edition. The library runs a **compatibility check** against 106-04 (for
older files) or the baseline (for newer ones) and names it so in every
report, which reconciles ADR-0016's "unknown editions are not guessed" with
its useful checking of old files.

## 10. Refinement T7: edit transactions and the checksum stamp

From the team design review, priority T7 (`docs/ROADMAP.md`). Applied
2026-09-26 with three questions left open for the owner (F1–F3); recorded
in ADR-0029, which refines ADR-0007 and ADR-0014. Point 10 of section 2
(edits as a patch list, byte-faithful output) stands; this section defines
what an edit may target, how a set of edits is applied, and how the stamp is
computed.

![An edit set is one transaction; the stamp is its last step](diagrams/edit-transaction.svg)

*Transactions and the stamp.* A set of edits is validated as a whole,
applied atomically as a new revision, the derived state it affects is
rebuilt, and the emitted bytes are re-read and verified; any rejection
refuses the whole set. A stamp is the last step and hashes the final bytes,
including any line break the insertion added.

### 10.1 What an edit targets

An edit names an item by an **item ID bound to one document revision**, or
by code name. The whole set is rejected, with a finding naming the edits and
items involved, when:

| Case | Rule |
|------|------|
| Overlapping edits | two edits touch the same bytes or the same item; inserts at one position keep the set's order |
| Stale item ID | the ID belongs to another revision; it is never re-targeted |
| Duplicate attributes | a code name matches several items and the edit names neither one item nor "every occurrence" |
| Renumbering collision | the target index is in use and the same set does not move its occupant (a swap is validated as a whole) |
| Extensions of a removed attribute | **open — follow-up F2** |

Counters change only when the set says so; otherwise the transaction's
findings suggest the counter edit (ADR-0007).

### 10.2 The transaction

1. **Validate** the whole set: targets, the rules of 10.1, and well-formed
   new text (code names parse; values contain no `;`, §9.4.2).
2. **Apply** it atomically, producing a new revision; the original is
   untouched, and on any failure nothing changes.
3. **Rebuild** the derived state it affects: index, link graph, derivation
   graph, effective values. The first implementation rebuilds in full; the
   benchmarks decide whether incremental rebuilding is worth it.
4. **Verify** by re-reading the emitted bytes: they must hold exactly the
   intended attributes, and a stamped checksum must verify.

### 10.3 The stamp

The digest excludes only the text "from "G\SHA:" to the following ";"
(inclusive)" (Table 9-2; Chapter 6 §6.2.3.11 f). Anything written around the
item — a line break added on insertion — is hashed, so the stamp is the
**last step of a transaction**: emit the final bytes with the `G\SHA` item in
place, using the document's own separator and line ending; compute SHA-256
over every byte outside the item; write the value inside the item, which
cannot change the digest; verify.

### 10.4 `G\SHA` policy

| Case | Behaviour | Register |
|------|-----------|----------|
| `g\sha` in another case | it is the code name (§9.4.2); the same text inside a value is not (D6) | INT-018 |
| no following semicolon | error; verification "malformed"; a terminator is suggested | INT-019 |
| upper-case hex that otherwise matches | a match, with a warning ("lower-case", Chapter 6) | INT-020 |
| two or more items | **open — follow-up F1** | INT-021 |

### 10.5 The generator

With sufficient input, generated TMATS validates for the selected edition.
With insufficient input the generator returns an **incomplete draft** and a
finding for each missing input; it never presents the draft as valid and
never invents values (L1-WRT-006, UC-13).

### 10.6 Open follow-ups

F1 (two or more `G\SHA` items), F2 (removing an attribute that X extensions
point to), and F3 (suggesting `:` where `=` was typed, as in Chapter 6's
`.TMATS WRITE` example `G\DSI\N=18;`) are open in `docs/ROADMAP.md`,
...
H group's two built-in attributes (ADR-0027); the TMATS edition and the
recording-format version kept apart, with every validation report naming the
edition applied and its basis (ADR-0028); edits as validated, atomic,
verified transactions over targets bound to a revision, with the checksum
